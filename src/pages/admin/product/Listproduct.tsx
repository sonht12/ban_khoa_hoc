    ...
    const columns = [
      {
        ...
        dataIndex: "name",
        key: "name",
        render: (text: any,{ key: _id }: any) => <div className="name-style text-[16px]"><Link to={`/admin/product/detail/${_id}`}>{text}</Link></div>,
      },
      {
        title: "Giá",
        dataIndex: "price",
    ...
    return (
      <div>
        <header className="mb-4 flex justify-between items-center">
          <h2 className="font-bold text-3xl mb-8">Quản lý khóa học</h2>
          <button className="bg-green-700 hover:bg-green-600 hover:text-white  text-white font-bold py-1 px-4 border border-green-600 rounded w-48 h-10 flex items-center ml-[52%]">
            <Link
        ...
        {isRemoveSuccess && (
          <Alert message="Xóa Thành Công Bạn Nhóe" type="success" />
        )}
        {isLoading ? (
          <Skeleton />
        ) : (
